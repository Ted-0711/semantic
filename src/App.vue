<template>
  <div id="app">
        ...
        <el-main id="main">
          <el-row>
            <el-input
              ...
              style=""
              v-show="resultRadio != 4 || quatList.length == 0"
            >
            </el-input>
            <el-table
              height="327"
            ...
            >
              ...
              <el-table-column prop="op" label="op" align="center">
              </el-table-column>
              <el-table-column prop="arg1" label="arg1" align="center">
              </el-table-column>
              <el-table-column prop="arg2" label="arg2" align="center">
              </el-table-column>
          ...
          <el-row>
            <div style="margin-top: 16px">
              ...
              <el-radio
                v-model="resultRadio"
                label="3"
                :disabled="grammar_stat == 0"
                >语法规则内容</el-radio
              >
              <el-radio
                v-model="resultRadio"
                label="4"
                :disabled="grammar_stat < 2"
                >语法语义分析结果</el-radio
              >
            </div>
          </el-row>
    ...
    <el-row v-show="resultRadio == '2'">
      <el-col :span="12">
        <h2>符号表</h2>
        <el-table height="365" :data="idList" style="width: 100%">
      ...
      <el-col :span="12">
        <h2>常量表</h2>
        <el-table height="365" :data="constList" style="width: 100%">
    ...
    <el-row v-show="resultRadio == '3'">
      <el-col :span="24">
        <h2>LR(1)分析表</h2>
        <el-table height="365" :data="actionGotoContent" style="width: 100%">
          <el-table-column
...
import { errString } from "./grammatical_analysis/grammar";
import {
  LR1,
  errString_2,
  errString_3,
} from "./grammatical_analysis/grammatical_analysis";
...
export default {
  ...
  methods: {
    clearLexResults(nPre = true) {
      ...
      // 语法、语义分析
      this.semanticResult = "";
      this.quatList.splice(0, this.quatList.length);
      this.grammarAnalysisProc.splice(0, this.grammarAnalysisProc.length);
    },
    clearGrammarResults() {
      // 读入语法规则文件
      this.grammarResult = "";
      this.actionGotoTitle.splice(0, this.actionGotoTitle.length);
      this.actionGotoContent.splice(0, this.actionGotoContent.length);
      // 语法、语义分析
      this.semanticResult = "";
      this.quatList.splice(this.quatList.length);
      this.grammarAnalysisProc.splice(0, this.grammarAnalysisProc.length);
    },
    // 获取源代码
    getSourceCode() {
    ...
    lexicalAnalysis(word) {
      ...
      if (this.IsLetter()) {
        ...
        word.prop = value;
      } else if (this.singleSym.indexOf(this.ch) != -1) {
        this.errFlag = false;
        word.type = this.ch;
      } else if (this.doublenESym.indexOf(this.ch) != -1) {
        this.errFlag = false;
        this.Concat();
        this.GetChar();
      ...
      } else if (this.EqualSym.indexOf(this.ch) != -1) {
        this.errFlag = false;
        this.Concat();
        this.GetChar();
        if (this.ch == "=") {
    ...
    grammaticalAnnalysis() {
      try {
        ...
        this.quatList.splice(0, this.quatList.length);
        this.grammarAnalysisProc.splice(0, this.grammarAnalysisProc.length);
        let parseArray = this.lr1.parser(
        ...
        for (let i = 0; i < parseArray.length; ++i) {
          ...
          for (let j = 0; j < this.grammarAnalysisTitle.length; ++j) {
            item[this.grammarAnalysisTitle[j]] = parseArray[i][j];
          }
          this.grammarAnalysisProc.push(item);
        }
        if (errString_3.length != 0) {
          this.semanticResult = "语义分析错误：\n" + errString_3;
          this.$message({
            showClose: true,
            message: "语法分析完成",
            type: "success",
          ...
          this.quatList = this.lr1.semanticAnalysis.GenQuat();
          this.$message({
            showClose: true,
            message: "语法语义分析完成",
            type: "success",
        ...
        if (error < 6) {
          this.$message({
            showClose: true,
            message: "Error: " + errString_2,
            type: "error",
          });
        } else {
          this.$message({
            showClose: true,
            message: "Error: " + errString_3,
            type: "error",
          });
        }
      }
      ...
      if (this.chartInstance == undefined) {
        ...
        initOption = {
          ...
          series: [
            {
              type: "tree",
              data: [this.lr1.nodeList[this.lr1.nodeList.length - 1]],
              orient: "vertical",
  ...
  mounted() {
    ...
    ipcRenderer.on("file-content", (e, msg) => {
      ...
      this.stat = 0;
      this.grammar_stat = this.grammar_stat >= 1 ? 1 : 0;
      if (this.resultRadio != "3") this.resultRadio = "1";
    ...
    ipcRenderer.on("grammar-content", (e, msg) => {
      if (msg.errFlag) {
        ...
        try {
          ...
          let actionGotoTable = this.lr1.genTableArray();
